...
public class XLSConverter {
    ...
    public XLSConverter(ConversionConfig conversionConfig) {
    ...
    public List<DataRow> convert(DirectoryFile file) throws IOException {
        ...
        poifsFileSystem = new POIFSFileSystem(new FileInputStream(file.getFilePath().toFile()));
    ...
    private List<DataRow> processWorkbook() throws IOException {
        ...
        if(conversionConfig.getGetAllSheets()) {
            ...
            while (iterator.hasNext()) {
                dataRowList.addAll(processSheet((HSSFSheet) iterator.next()));
            }
        }else {
    ...
    // Loop over sheets
    private List<DataRow> processSheet(HSSFSheet sheet) {
